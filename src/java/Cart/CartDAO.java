...
public class CartDAO {
    ...
    ResultSet rs;
    public boolean addToCart(String cartId,String idProduct, float price, float totalPrice, int quantity,String image,String userID,String invId) throws SQLException, ClassNotFoundException {
    String sql = "INSERT INTO tblCart (cartId, idProduct, price, totalPrice, quantity,image,userID,invId) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    try {
        ...
        rs = ps.executeQuery();
        while (rs.next()) {
            cart = new CartDTO(rs.getString("cartId"), 
    ...
    public CartDTO getCartById(String idProduct, String userID) throws ClassNotFoundException {
        String sql = "SELECT * FROM tblCart WHERE idProduct = ? and userID =?;";
        CartDTO cart = null;
        try {
            connection = DBUtils.getConnection();
            ps = connection.prepareStatement(sql);
            ...
            if (rs.next()) {
                ...
                cart = new CartDTO(cartId, idProduct,invId,userID, image, price, totalPrice, quantity);
            }
        } catch (SQLException ex) {
            // Handle your exception here
        } finally {
    ...
    public CartDTO getCartById(String cartId) throws ClassNotFoundException {
        ...
        try {
            ...
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                String idGlasses = rs.getString("idProduct");
                ...
                int quantity = rs.getInt("quantity");
                String image = rs.getString("image");
                cart = new CartDTO(cartId, idGlasses,invId,userID, image, price, totalPrice, quantity);
            }
        } catch (SQLException ex) {
            // Handle your exception here
        } finally {
